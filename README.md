...
![Clothing try-on showcase — six adversarial garments](docs/showcase/clothing_showcase.jpg)

*Six of the hardest garments in the stress set, each a real try-on of the same
synthetic person: satin gown with 3D appliqué, off-shoulder bubble hem, a denim
track jacket **layered over** the tee, a structured moto jacket with dangling
laces, a large-paillette sequin top, and a wrap mini-skirt that leaves the legs
bare. Identity and background are the original photo's pixels in every frame.*
...
**Jewelry** (each a real app output):
...
garments (oxford, black dress, jeans shown below; breton + wrap dress above), and
the adversarial showcase is at the top of this README.

| White Oxford | Black A-line | Straight Jeans |
| --- | --- | --- |
| ![oxford](docs/demo/result_white_oxford.jpg) | ![black dress](docs/demo/result_black_dress.jpg) | ![jeans](docs/demo/result_jeans.jpg) |
...
**Jewelry:** 10 items with royalty-free product photos — The Met **CC0**
...
[`catalog.json`](backend/catalog/catalog.json). **Clothing:** 5 items with
product images generated by Nano Banana for this project (disclosed per-item).
**Adversarial stress garments** in `eval/stress/refs/` are third-party product
photos used solely as local evaluation stress assets, not catalog items.
